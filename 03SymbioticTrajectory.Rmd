...
Metagene in pseudotime
```{r}
symbiotic_mono <- readRDS("../RNAi_wrap/data/RNAi_symbiotic_mono.RDS")
t.goodCellScores <- as.data.frame(t(goodCellScores))
colnames(t.goodCellScores) <- paste0("Metagene_",colnames(t.goodCellScores))
...
#pdf("figures/pseudotime_Metagene44.pdf",height=3,width=7)
ggplot(pData(symbiotic_mono), aes(x = Pseudotime, y = Metagene_44,color=sample))  +stat_summary_bin(fun.data = "mean_cl_boot", geom = "crossbar", binwidth = 1)+theme_classic()
#dev.off()
plot_cell_trajectory(symbiotic_mono, color_by="Metagene_44 ",show_tree = F,show_branch_points=F,show_state_number =F ,cell_size = 0.8)+scale_color_gradientn(colours = c("gray","red")) + facet_wrap(~sample)

```
```{r}
ggplot(pData(symbiotic_mono)) +geom_freqpoly(aes(x=Pseudotime, y= ..density.., colour = sample),binwidth = 1) +stat_summary_bin(aes(x = Pseudotime, y = Metagene_44,color=sample),fun.data = "mean_cl_boot", geom = "crossbar",  binwidth = 1) +theme_classic()
```
...
# Mapping previous 
```{r}
transition1_marker <-read.table("transition1_ID.txt",header = F)
pre_endosymbiotic_marker <-read.table("pre_endosymbiotic_ID.txt",header = F)
transition1_marker <- left_join(transition1_marker,id2symbol,by=c("V1"="ID"))
transition1_marker <- left_join(transition1_marker,gene_to_cluster, by= c("symbol"="gene_short_name"))
